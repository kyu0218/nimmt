function ret=cp_MCr_row(p) % もんてかるろ
...
for j = 1:4 % 各列に対する試行(このターンにj行目を引き取ったなら)
    ...
    parfor n = 1:N % 試行回数
        ...
        pl_i=usecard_i(1,2); cd_i=usecard_i(1,1);
        row = j; %row = input_row(pl_i);
        field_i(row,6) = cd_i;
        if pl_i == p
